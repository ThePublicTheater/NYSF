...
namespace Ambitus
{
	public class LineItem
	{
		...
		public LineItem(int? _li_seq_no, int? _li_no, int? _order_no, int? _pkg_no,	int? _perf_no,
				short? _priority, short? _zone_no, string _alt_upgrd_ind, int? _pkg_li_no,
				bool? _primary_ind, string _perf_code,	DateTime? _perf_dt, string _perf_desc,
				string _pkg_code, string _pkg_desc, string _facility_desc, short? _db_status,
				string _notes, int? _zmap_no, string _text1, string _text2, string _text3,
				string _text4, short? _prod_type, string _prod_type_desc, int? _category,
				int? _assoc_customer_no, short? _season_no, string _season_desc, bool? _ga_ind,
				int? _facil_no,	DataTable sliTable)
		{
			Id = _li_seq_no;
			Number = _li_no;
			OrderId = _order_no;
